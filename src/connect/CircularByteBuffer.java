 ...
 * <p> Using this class is a simpler alternative to using a PipedInputStream and
 * a PipedOutputStream. PipedInputStreams and PipedOutputStreams don't support
 * the mark operation, don't allow you to control buffer sizes that they use, and
 * have a more complicated API that requires instantiating two classes and
 * connecting them. <p> This class is thread safe.
 *
...
public class CircularByteBuffer {
     ...
     * @since ostermillerutils 1.00.00
     */
    private final static int DEFAULT_SIZE = 8192 * 16;
    /**
    ...
    /**
     * The circular buffer. <p> The actual capacity of the buffer is one less
     * than the actual length of the buffer so that an empty and a full buffer
     * can be distinguished. An empty buffer will have the markPostion and the
     * writePosition equal to each other. A full buffer will have the
     * writePosition one less than the markPostion. <p> There are three important
     * indexes into the buffer: The readPosition, the writePosition, and the
     * markPosition. If the InputStream has never been marked, the readPosition
     * and the markPosition should always be the same. The bytes available to be
     * read go from the readPosition to the writePosition, wrapping around the
     * end of the buffer. The space available for writing goes from the write
     * position to one less than the markPosition, wrapping around the end of the
     * buffer. The bytes that have been saved to support a reset() of the
     * InputStream go from markPosition to readPosition, wrapping around the end
     * of the buffer.
     *
     * @since ostermillerutils 1.00.00
     */
